...
function getAuth() {
	const keyEnv = process.env.GOOGLE_SERVICE_ACCOUNT_JSON;
	const authOptions = { scopes: ['https://www.googleapis.com/auth/drive.readonly'] };

	// Support both raw JSON string (Vercel env vars) and file path
	if (keyEnv && keyEnv.trim().startsWith('{')) {
		authOptions.credentials = JSON.parse(keyEnv);
	} else {
		authOptions.keyFile = path.resolve(keyEnv);
	}
